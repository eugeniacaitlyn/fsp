...
<body>
    ...
    $param = "%$keyword%";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td>
                    <a href ='/fsp/projek/me/admin/event/EditEvent.php?id=$idevent'>
                        <button class='icon-button edit'>
                            <i class='bx bxs-edit-alt'></i>
                        </button>
                    </a>
                    <a href ='/fsp/projek/me/admin/event/eventDelete.php?id=$idevent' onclick='return confirm(\"Apakah Anda yakin ingin menghapus game ini?\")'>
        ...
        if ($page>1){
            $prev = $page-1;
            echo "<a class='pagination-btn' href='event.php?p=$prev&keyword=$keyword&rows=$perpage'><</a> ";
        }
        ...
        echo "<a class='pagination-btn' href='event.php?p=$totalpage&keyword=$keyword&rows=$perpage'>Last</a> ";
        if ($page<$totalpage){
            $next = $page+1;
            echo "<a class='pagination-btn' href='event.php?p=$next&keyword=$keyword&rows=$perpage'>></a> ";
        }
        echo "<br><br>";
        echo '    </main>';
        echo '</div>';
